...
\author{Santiago Hamwee, Florencia Montti, Javier Spina}
...
\begin{frame}{Cómo representamos al usuario}
  \begin{itemize}
    \item El \textbf{historial} y la \textbf{query} aportan información distinta:
      \begin{itemize}
        \item historial = \emph{qué le gustó}: el \textbf{centroide}, el promedio de las distribuciones de tópicos de lo que vio;
        \item query = \emph{qué quiere ahora}: su distribución de tópicos (\texttt{lda.transform}).
      \end{itemize}
    \item Perfil del usuario = \textbf{promedio} de ambas distribuciones, con el mismo peso. Es la misma idea de representar promediando vectores de la Unidad 4, aplicada a distribuciones de tópicos.
